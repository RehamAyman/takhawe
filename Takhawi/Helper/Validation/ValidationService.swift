...
struct ValidationService {
    ...
    static func validate(providerName: String?) throws -> String {
        guard let providerName = providerName, !providerName.isEmpty else {
            throw ValidationError.emptyproviderName
        }
        guard providerName.count > 2 else {
            throw ValidationError.shortproviderName
        }
        guard providerName.count < 61 else {
            throw ValidationError.longproviderName
        }
        return providerName
    }
    ...
    static func validate(priceFrom: Int, priceTo: Int) throws {

        if priceTo < priceFrom {
            throw ValidationError.priceRange
        }
    }
    ...
    static func validate(projectTitle: String?) throws -> String {
        guard let projectTitle = projectTitle, !projectTitle.isEmpty else {
            throw ValidationError.projectTitle
        }
        return projectTitle
    }

    static func validate(projectCategory: String?) throws -> String {
        guard let projectCategory = projectCategory, !projectCategory.isEmpty else {
            throw ValidationError.projectCategory
        }
        return projectCategory
    }

    static func validate(projectDetils: String?) throws -> String {
        guard let projectDetils = projectDetils, !projectDetils.isEmpty else {
            throw ValidationError.projectDetilsempty
        }
        return projectDetils
    }
    static func validate(priceFrom: String?) throws -> String {
        guard let priceFrom = priceFrom, !priceFrom.isEmpty else {
            throw ValidationError.emptyPriceFrom
        }
        return priceFrom
    }

    static func validate(priceTo: String?) throws -> String {
        guard let priceTo = priceTo, !priceTo.isEmpty else {
            throw ValidationError.emptyPriceTo
        }
        return priceTo
    }

    static func validate(expictedTime: String?) throws -> String {
        guard let expictedTime = expictedTime, !expictedTime.isEmpty else {
            throw ValidationError.expictedDurationempty
        }
        return expictedTime
    }
    // MARK: - Passwords -
    ...
    static func validate(ConmfirmPassword: String?) throws -> String {
        guard let ConmfirmPassword = ConmfirmPassword, !ConmfirmPassword.isEmpty else {
            throw ValidationError.emptyConfirmPassword
        }
        guard ConmfirmPassword.count > 7 else {
            throw ValidationError.shortPasswordConfirmation
        }
        return ConmfirmPassword
    }
    ...
    static func validate(massgeTitle: String?) throws -> String {
        guard let massgeTitle = massgeTitle, !massgeTitle.isEmpty else {
            throw ValidationError.Titleofthemessage
        }
        return massgeTitle
    }

    static func validate(massgeDetils: String?) throws -> String {
        guard let massgeDetils = massgeDetils, !massgeDetils.isEmpty else {
            throw ValidationError.detilsOfMessage
        }
        return massgeDetils
    }

    static func validate(reportTitle: String?) throws -> String {
        guard let reportTitle = reportTitle, !reportTitle.isEmpty else {
            throw ValidationError.emptyReportTitle
        }
        return reportTitle
    }

    static func validate(reportDetils: String?) throws -> String {
        guard let reportDetils = reportDetils, !reportDetils.isEmpty else {
            throw ValidationError.emptyReportMessage
        }
        return reportDetils
    }

    static func validate(nameOfAccountHolder: String?) throws -> String {
        guard let nameOfAccountHolder = nameOfAccountHolder, !nameOfAccountHolder.isEmpty else {
            throw ValidationError.emptynameOfAccountHolder
        }
    ...
    static func validate(accountNumber: String?) throws -> String {
        guard let accountNumber = accountNumber, !accountNumber.isEmpty else {
            throw ValidationError.emptyaccountNumber
        }

        guard accountNumber.count > 14 && accountNumber.count < 24 else {
            throw ValidationError.incorrectAccountNumber
        }
...
//    static func validate(selectProduct: [ProductCart]?) throws -> [ProductCart] {
...
//    }
    ...
    static func validate(storeName: String?) throws -> String {
        guard let storeName = storeName, !storeName.isEmpty else {
            throw ValidationError.storeName
        }
        return storeName
    }
    ...
    static func validate(deliveryTime: String?) throws -> String {
        guard let deliveryTime = deliveryTime, !deliveryTime.isEmpty, deliveryTime != "0"
        else {
