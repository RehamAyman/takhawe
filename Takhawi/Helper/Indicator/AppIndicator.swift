...
class AppIndicator {
    ...
    private func handelInitialView(isGif: Bool) {
    ...
    func show(isGif: Bool) {
        ...
        let window = UIApplication.shared.windows.first { $0.isKeyWindow }
        guard let window = window else {return}
        window.topViewController()?.startAnimating()
//        self.handelInitialView(isGif: isGif)
...
//        }
    ...
    func dismiss() {
        ...
        let window = UIApplication.shared.windows.first { $0.isKeyWindow }
        guard let window = window else {return}
        window.topViewController()?.stopAnimating()
...
extension UIViewController: NVActivityIndicatorViewable {
    ...
    func showProgress() {
        NVActivityIndicatorView.DEFAULT_COLOR = .appColor(.MainColor)!
